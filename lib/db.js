...
// Notes are shared by every team, so they live at the top level, not under a team.
...
export function subscribeNotes(cb, onError) {
  // Ordered client-side, for the same pending-timestamp reason as sprints.
  return listen(notesCol(), (snap) => cb(snap.docs.map((d) => ({ id: d.id, ...d.data() }))), onError)
}
...
/** `team` is the workspace it was written from, kept only to show where it came from. */
export async function createNote(data, user, team) {
  validateNote(data)
...
export async function setNotePinned(noteId, pinned) {
  await updateDoc(noteDoc(noteId), { pinned: Boolean(pinned) })
}

/** A member confirms they have read a guideline, or takes that back. */
export async function acknowledgeNote(noteId, uid, read = true) {
  await updateDoc(noteDoc(noteId), { ackBy: read ? arrayUnion(uid) : arrayRemove(uid) })
}
