...
export async function ensureUserDoc(user, displayName) {
  ...
  // Only an explicit name (sign-up) may replace the stored one, otherwise a
  // rename from Settings would be undone on the next sign-in.
  const patch = displayName ? { ...base, displayName } : base
